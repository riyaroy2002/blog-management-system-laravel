...
namespace App\Http\Controllers\Authentication\Author;
...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        $validated = Validator::make($request->all(), [
            ...
            'contact_no'         => ['required', 'digits:10', 'regex:/^[0-9]{10}$/', Rule::unique('users', 'contact_no')],
        ...
        try {
            ...
            $token = Str::random(64);
            $verifyLink = url('/author/verify-email/' . $token . '?email=' . $validated['email']);
            $data         = [];
            $email_sent   = '';
            ...
            DB::commit();
            return redirect()->route('author.verify-email')->with('success', 'User created successfully');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    {
        return view('auth.author.verify-email');
    }
    ...
    public function verifyEmail($token)
    {
        ...
        if (!$user) {
            return redirect()->route('author.login')->with('error', 'Invalid or expired verification link.');
        }
        ...
        return redirect()->route('author.login')->with('success', 'Email verified successfully. You can now login.');
    }
    ...
    public function resendEmailLink(Request $request)
    {
        ...
        $token = Str::random(64);
        $verifyLink = url('/author/verify-email/' . $token . '?email=' . $request->email);
        $data         = [];
        $email_sent   = '';
        $data['link'] = $verifyLink;
        $data['name'] = $user->name;
    ...
    public function login(Request $request)
    {
        ...
        try {
            ...
            $validated = $validated->validated();
            $user = User::where('email', $request->email)->where('role', 'author')->first();
            if (!$user) {
    ...
    {
        return view('auth.author.forgot-password');
    }
    ...
    public function sendResetLink(Request $request)
    {
        ...
        $resetLink    = url('/author/reset-password/' . $token . '?email=' . $request->email);
        $data         = [];
        $data['link'] = $resetLink;
        if (filter_var($request->email, FILTER_VALIDATE_EMAIL)) {
    ...
    {
        return view('auth.author.reset-password');
    }
    ...
    public function resetPassword(Request $request)
    {
        ...
        PasswordResetToken::where('email', $request->email)->delete();
        Session::forget('email');
        return redirect()->route('author.login')->with('success', 'Password updated successfully. Please login.');
    }
